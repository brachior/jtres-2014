...
\conferenceinfo{JTRES'14,}{October 13 - 14 2014, Niagara Falls, NY, USA}
\CopyrightYear{2014} % Allows default copyright year (20XX) to be over-ridden - IF NEED BE.
\crdata{978-1-4503-2813-5/14/10}  % Allows default copyright data (0-89791-88-6/97/05) to be over-ridden - IF NEED BE.
...
\section{Overview}
  ...
  \subsection{\ANDROID}
   ...
   ~\cite{ieee-butler-android-landscape}\cite{ieee-paulson-shift-dynamic-languages}.
    ...
    \subsubsection{Differences between the JVM and \DALVIK}
      ...
      The file is made of a series of tables which describe various information, the code of the class and some other things.
  ...
  \subsection{Java Specification Request 292}
    ...
    \subsubsection{MethodHandle / MethodHandles}
      ...
      Moreover, the semantics of invokedynamic allow the runtime to link again, by calling the method setTarget
    ...
    \subsubsection{CallSite}
      ...
      \end{enumerate}
      A non-constant call site may be linked again by changing its target, calling ``setTarget'' on the CallSite.
      The new target must have the same type as the previous one.

\section{\ANDROID 292}
  The \JSR add new features and new ways of coding with Java (lambdas in Java 8 use invokedynamic).
  If \ANDROID does not follow Java updates, Java developers will see \ANDROID as an old Java platform.
  \ANDROID must implement this \Jsr to maintain its relevance.
...
\label{newDEX}
  ...
  \subsection{opcodes / constants}
    ...
    A call to ``invokeExact'' or ``invoke'' uses an invokevirtual and needs at runtime to determinate if it is a real invokevirtual or a specific call to ``invoke'' or ``invokeExact''.
    We choose to introduce two specific instructions to avoid this check at runtime.
%     ``invokeExact'' and ``invoke'' which require that the first parameter is a MethodHandle.
...
\label{implD}
    ...
    \subsubsection{invoke-dynamic}
      ...
      at the same time, this code can be executed concurrently.
      In order to be sure that every thread sees the same CallSite
...
%       \end{figure}
    ...
    \label{combiners}
      ...
      Standard edition Java defines a large number of combiners and some may be introduced in the next version
...
%   \softraggedright
  ...
  \bibitem{bb-dalvik}
  BitBucket, ``Dalvik 292'' - \url{https://bitbucket.org/jpilliet/dalvik-292}.

  \bibitem{bb-android}
  BitBucket, ``Android 292'' - \url{https://bitbucket.org/jpilliet/android-292}.
