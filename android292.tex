...
\label{implD}
    ...
    \subsubsection{invoke-dynamic}
      ...
      after the initialisation, the read inside the callsite table is a volatile read and the write
      is done using a compare-and-set with null as expected value.
      If the compare-and-set fail, we re-read the callsite table at the right index again with the volaile semantics
...
\section{Conclusion}
  ...
  modifyies the C interpreter to execute direct method handles and invokers, uses a Java interpreter for the support
