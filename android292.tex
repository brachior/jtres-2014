  ...
  \subsection{Java Specification Request 292}
    ...
    \subsubsection{MethodHandle / MethodHandles}
      ...
      \footnote{There is also a minor difference between the generic call semantics and the call semantics of
        the Java Language Specification, if the method is a varargs and the call as the same arity as the method.
...
\label{newConst}
  ...
  \subsection{New invoke instructions}
    ...
    \subsubsection{invoke-dynamic}
      ...
      This entry in the invoke-dynamic constant pool contains the following information: bootstrap method, bootstrap arguments,
...
\label{implD}
    ...
    \subsection{MethodHandle}
      ...
      A MethodHandle is described by: the declaring class; the method type; the kind and the slot corresponding to the method/field/\dots
      ...
      (but usually in a slow path). Those method handle does not require any action by the VM
      thus are fully created in Java.
