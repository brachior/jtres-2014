...
\title{Android 292}
\subtitle{invokedynamic in Android}
...
\begin{abstract}
\fixme{bla bla bla bla bla bla bla bla bla
...
\keywords
android, java, jsr 292, invokedynamic, embedded, dalvik
...
%         \centering \includegraphics[width=\columnwidth]{structure-jar-apk.png}
  ...
  \subsection{Java Specification Request 292}
    ...
    \subsubsection{invoke-dynamic and MethodHandle}
      ...
      \begin{figure}[!h]
        \centering \input{figures/snip_foo_bsm}
    ...
    \subsubsection{MethodType}
      ...
      \begin{figure}[!h]
        \centering \input{figures/snip_foo_methods}
    ...
    \subsubsection{MethodHandle / MethodHandles}
      ...
      \begin{figure}[!h]
        \centering \input{figures/snip_foo_callsite}
      ...
      \begin{figure}[!h]
        \centering \input{figures/snip_foo_change}
...
%       \centering \includegraphics[width=.5\columnwidth]{structure-apk-292.png}
    ...
    \begin{figure}[!h]
      ...
      \centering \input{figures/snip_dvm_struct_bsm}
...
\label{implD}
    ...
    \subsection{MethodHandle}
      ...
      And the last part ("invoker") is formed by special method handles which need a specific manipulation in the Dalvik interpreter.
      Each part is treated differently by the Dalvik interpreter.

      We distinguish all kinds of method handle with several binary masks.

      \noindent \fixme{detailler completement (masks) ?}\\
      \fixme{SlotAccessor ?}\\
      \fixme{vminfo (field/method) ?}
    ...
    \subsubsection{invoke-dynamic}
      ...
      \begin{figure}[!h]
        \centering \input{figures/snip_meth_bsm}
      ...
      \begin{figure}[!h]
        \centering \input{figures/snip_meth_iwa}
...
%       Une partie des method handles permet la manipulation des arguments, on les appelle: ``combiners''.
%       Le combiners possèdent d'autres method handles et au moins un method handle direct.
%       De ce fait, on peut les repr\'esenter sous forme d'arbres, on parlera ici d'arbres de method handles.

      Execute a combiner is often equivalent to execute many.
...
%       Ex\'ecuter un combiner est donc souvent \'equivalent \`a en ex\'ecuter plusieurs.
%       Étant donn\'e que les combiners ne peuvent pas \circ{e}tre des appels finaux,
%       il n'est donc pas interressant de faire des appels r\'eels, c'est \`a dire mettre sur la pile d'ex\'ecution,
%       chaque combiner pour enfin appeller le method handle direct, qui lui est un appel r\'eel.
...
%       Pour ex\'ecuter les method handles directs, nous devons utiliser les instructions ``invoke-exact'' et ``invoke-generic''.
%       Mais elles ne sont pas accessibles via l'interpreter, nous devons donc remonter en Java pour les appeller.
%       Pour \'eviter de passer de l'interpreteur en C \`a une m\'ethode Java pour chaque combiner,
%       nous avons donc choisi de faire un interpreteur, r\'eserv\'e \`a l'ex\'ecution des combiners, \'ecrit en Java (Figure \ref{implINTERP}).

      \begin{figure}[!h]
        \centering \input{figures/snip_meth_interp}
...
%       It is implemented using a continuation mechanism (context) to treat the return value.
    ...
    \subsection{Invokers}
      ...
      Among method handles, we call "invoker" those which apply a specific call to a method handle passed as a parameter.
      This call can be a combiner (``spreadInvoker'' \fixme{c'est le seul ?}) and also an ``invoke'' (``exactInvoker'' and ``invoker'').
      The ``spreadInvoker'' is simulated by a method handle \fixme{lequel ?}.
      Concerning the ``exactInvoker'' and the ``invoker'', they simulate respectively a call to the method ``invokeExact'' and ``invoke''.
      We treat these two invoker directly in the Dalvik interpreter, there are four cases:
%       Parmi les method handles, on appelle ``invoker'' ceux qui appliquent un appel particulier \`a un method handle pass\'e en param\`etre.
%       Cet appel peut correspondre \`a un combiner mais aussi \`a un ``invoke'' (``exactInvoker'' et ``invoker'').
%       Les appels \`a un combiner peuvent \^etre simul\'es par un method handle.
%       Et en ce qui concerne les appels \`a un ``invoke'', ils simulent un appel \`a la m\'ethode ``invoke'' ou \`a la m\'ethode ``invokeExact''.
%       Il faut traiter ces deux cas de fa\c{c}on particuli\`ere.
%       Nous devons intervenir au niveau de l'interpreteur, quatre possibilit\'es s'offrent \`a nous:
      \begin{enumerate}
        \item invoke $\rightarrow$ exactInvoker: an ``invoke-generic'' have to simulate an ``invoke-exact''.
              Because ``invoke-generic'' have the same behaviour than ``invoke-exact'',
              we can adapt the arguments (change current method handle, shift arguments, \dots) and back in the instruction code.
        \item invokeExact $\rightarrow$ exactInvoker: an ``invoke-exact'' have to simulate an ``invoke-generic''.
              The instructions are identicals so we can adapt arguments and back in the instruction code.
        \item invoke $\rightarrow$ invoker: an ``invoke-generic'' have to simulate an ``invoke-generic''.
              It is the same idea than previously.
        \item invokeExact $\rightarrow$ invoker: an ``invoke-exact'' have to simulate an ``invoke-exact''.
              This case is more difficult because an ``invoke-exact'' cannot do the ``invoke-generic'' work.
              We adapt the arguments and if the method type is correct, we do an ``invoke-exact''
              else we call the Java method ``invokeWithArguments'' which will call the ``invoke-generic'' instruction.
      \end{enumerate}
%       \begin{itemize}
%         \item invoke $\rightarrow$ exactInvoker: on demande \`a l'instruction ``invoke-generic'' d'executer le method handle avec un ``invoke-exact''.
%               Comme l'instruction ``invoke-generic'' se comporte comme celle d'``invoke-exact'' si le method type correspond, 
%               on peut adapter les arguments (changer le method handle courant, d\'ecaller les arguments, \dots)
%               et remonter dans l'instruction, tout en pr\'ecisant qu'on ne peut pas convertir les arguments.
%         \item invokeExact $\rightarrow$ exactInvoker: on demande \`a l'instruction ``invoke-exact'' d'executer le method handle avec un ``invoke-exact''.
%               Ici, on peut adapter les arguments et remonter dans l'instruction car les instructions sont identiques.
%         \item invoke $\rightarrow$ invoker: on demande \`a l'instruction ``invoke-generic'' d'executer le method handle avec un ``invoke-generic''.
%               Pour la m\^eme raison que pr\'ecedemment, on peut directement adapter les arguments et remonter.
%         \item invokeExact $\rightarrow$ invoker: on demande \`a l'instruction ``invoke-exact'' d'executer le method handle avec un ``invoke-generic''.
%               Ce cas est un peu plus compliqu\'e. En effet, l'instruction ``invoke-exact'' n'a pas toutes les fonctionnalit\'es de l'instruction ``invoke-generic''.
%               La solution ici est d'adapter les arguments et de remonter en Java en appelant la m\'ethode ``invokeWithArguments'' qui se chargera d'appeler l'instruction ``invoke-generic''.
%       \end{itemize}

      \fixme{autre chose a dire ?}
...
%   \softraggedright
  ...
  \JVM languages (Wikipedia) - \\ \url{en.wikipedia.org/wiki/List\_of\_JVM\_languages}
%   @ARTICLE{5676144, 
%   author={Butler, M.}, 
