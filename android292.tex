...
Java 7 has a new API as well as a new instructions set introduced by the \JSR.
This \Jsr simplifies the implementation of non-Java languages in the virtual machine.
It is not implemented in Android neither in the other embedded JVMs.
The difficulty is to produce the exact behaviour of the \JSR with strong constraints in terms of energy and computing power.

This paper describes our implementation of the \JSR in Dalvik.
We explain our modifications of the interpreter, written in C, and of the Java API
as well as our choice to adapt the opcodes specification in the Dalvik virtual machine.
One of the important parts of the \JSR is the arguments manipulation through the ``combiners''.
In order to treat them, our implementation includes a flexible interpreter, written in Java.

At the end, we do an execution time comparaison of the different kind of calls.
This comparaison shows that the new instructions are faster than the reflection
although the interpreter of combiners is slowed down by frequent objects creation.
\end{abstract}
