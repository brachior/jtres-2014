...
\usetikzlibrary{plotmarks,arrows,matrix,positioning,fit,calc}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{xspace}
\usepackage{hyperref}
...
%       \centering \includegraphics[width=.5\columnwidth]{structure-apk-292.png}
      \centering \resizebox{.55\linewidth}{!}{\input{figures/tikz_struct_new_apk}}
...
%       \fixme{detailler completement ? genre montrer les masques ?\\Parler du SlotAccessor ? et des slot/info ?}
...
%       It is implemented using a continuation mechanism (context) to treat the return value.
    ...
    \subsection{Invokers}
      ...
      Parmis les method handles, on appelle ``invoker'' ceux qui appliquent un appel particulier \`a un method handle pass\'e en param\`etre.
      ...
      \begin{itemize}
        \item invoke/exactInvoker - on demande \`a l'instruction ``invoke-generic'' d'executer le method handle avec un ``invoke-exact''.
        ...
        \item invokeExact/exactInvoker - on demande \`a l'instruction ``invoke-exact'' d'executer le method handle avec un ``invoke-exact''.
              Ici, on peut adapter les arguments et remonter dans l'instruction car les instructions sont identiques.
        \item invoke/invoker - on demande \`a l'instruction ``invoke-generic'' d'executer le method handle avec un ``invoke-generic''.
              Pour la m\^eme raison que pr\'ecedemment, on peut directement adapter les arguments et remonter.
        \item invokeExact/invoker - on demande \`a l'instruction ``invoke-exact'' d'executer le method handle avec un ``invoke-generic''.
