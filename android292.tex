...
\section{Overview}
  ...
  \subsection{Java Specification Request 292}
    ...
    \subsubsection{invoke-dynamic and MethodHandle}
      ...
      the string {\bf ``+''} and a MethodType corresponding to the descriptor ``(LObject;I)LObject;''.% (cf. \ref{FOOBSM}).
...
%       A method handle can be create by different factory methods depending to the kind of method handle.
...
\section{Android 292}
  ...
  To do this, the section \ref{newDEX} explain our new DEX format with a new header, classes modification
  \dots~and how we maintain compatibility with post-invokedynamic APKs.
  The section \ref{newConst} decribes the new constants, new instructions
  and .
  And we made an specific interpreter, written in Java, which executes method handle trees (combiners).
...
\label{newDEX}
  ...
  \subsection{constant-pools}
    ...
    \input{figures/snip_dvm_struct_mt}
    \input{figures/snip_dvm_struct_mh}
    \input{figures/snip_dvm_struct_indy}
    \input{figures/snip_dvm_struct_bsm}
...
\section{Implementation details}
    ...
    \subsection{MethodHandle}
      ...
      \begin{figure}[!h]
        \centering \resizebox{.7\linewidth}{!}{\input{figures/tikz_mh_hierarchy}}
...
%   \node (jar) [title] { .jar };
% 
%   \node (class) [title, below=of jar.west, typetag, xshift=2mm] { .class };
%   \node (jar_header) [below=of class.west, typetag] { Header };
% %   \node (dnc) [below=of jar_header.west, typetag] { DivideAndConquer };
% 
%   \node (class2) [below=of class.west, typetag] { .class };
% 
%   \node [draw=black!50, fit={(jar) (class) (class2) (jar_header)}] {};
% %   \node [draw=black!50, fit={(jar) (class) (class2) (jar_header) (dnc)}] {};
% 
% %   \node (dep) at (3cm, 0) [title] { Dependency };
% % 
% %   \node (da) [below=of dep.west, typetag, xshift=2mm] { Atomic };
% %   \node (jar_header) [below=of da.west, typetag] { Range };
% % 
% %   \node [draw=black!50, fit={(dep) (jar_header) (da)}] {};
% \end{tikzpicture}
% }
%         \caption{}
%       \end{figure}
...
%       \end{figure}
    ...
    \subsection{invoke-exact and invoke-generic}
  
    \subsection{invoke-dynamic}
      \subsubsection{bootstrap method caller}
      \label{bsm}

        When an ``invoke-dynamic'' is treated, the \JVM call the bootstrap method associated.% (Figure \ref{bsm}).
        ...
        To do that, we call the method ``invokeWithArguments''% (Figure \ref{IWA})
