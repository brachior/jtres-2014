...
\label{implD}
    ...
    \subsection{Invokers}
      ...
      Each invoker can be execute either by the ``invoke'' method or by the ``invokeExact'' method, so we have four call semantics.
      ...
      Then the interpreter re-executes itself using a goto.
      ...
      Reproduce the ``invoke-generic'' behaviour require to copy a lot of code so,
      we choose to do an up call to the method ``invokeWithArguments'' which calls the method ``invoke''.

      Method handles contains also others invokers (``spreadInvoker'' and ``dynamicInvoker'') but we can treat them like combiners.
...
\section{Related Works}
  ...
  Currently, the OpenJDK is the only one open-source implementation of the \JSR.
  As shown in several papers of John R. Rose and al. (JSR 292 spec leader), this JSR has often evolved.
  In 2009, he describes an implementation of the \Jsr, now obsolete\cite{vmil-rose-indy-2009}.
  In 2010, he publishes a paper presenting the implementation in the JDK 7\cite{pppj-rose-indy-2010}.
